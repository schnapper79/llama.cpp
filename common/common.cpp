...
static llama_control_vector_data llama_control_vector_load_one(const llama_control_vector_load_info & load_info) {
    ...
    LOG_INF("control vector load_one...\n");
    int32_t n_tensors;
    ...
    LOG_INF("control vector load_one took %ums\n", end - start);
    return result;
}

llama_control_vector_data llama_control_vector_load(const std::vector<llama_control_vector_load_info> & load_infos) {
    auto start = ggml_time_ms();
    LOG_INF("control vector load...\n");
    llama_control_vector_data result = { -1, {} };
    ...
    for (const auto & info : load_infos) {
        ...
        if (result.n_embd != -1 && (result.n_embd != cur.n_embd || result.data.size() != cur.data.size())) {
            LOG_INF("%s: control vector in %s does not match previous vector dimensions\n", __func__, info.fname.c_str());
            return result;
        }
    ...
    if (result.n_embd == -1) {
        LOG_INF("%s: no vectors passed\n", __func__);
    }

    auto end = ggml_time_ms();
    LOG_INF("control vector load time: %ums\n", end-start);
    return result;
}
